...
{
 pins = data;
cy_rslt_t rslt = cyhal_spi_init(&mSPI, pins->MOSI, pins->MISO, pins->SCK, pins->SS, NULL, 8,
CYHAL_SPI_MODE_10_MSB, false);
 if (CY_RSLT_SUCCESS == rslt)
 rslt = cyhal_spi_set_frequency(&mSPI, pins->frec);
 if (CY_RSLT_SUCCESS == rslt)
 rslt = cyhal_gpio_init(pins->dc, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG,
0u);
 if (CY_RSLT_SUCCESS == rslt)
 rslt = cyhal_gpio_init(pins->rst, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG,
1u);
 return rslt;
}
